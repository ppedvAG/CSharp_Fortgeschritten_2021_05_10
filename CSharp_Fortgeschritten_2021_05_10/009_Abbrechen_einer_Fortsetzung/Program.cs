...
public class CancellationExample
{
    ...
    public static async Task Main()
    {
        ...
        Task<int[]> task = Task.Run<int[]>(() => Genrate33List(token));

        Task<double> continuation = task.ContinueWith(a => a.Result.ToArray(), cts);

        Task<double> continuation1 = task.ContinueWith(
            async antecedent =>
            {
    ...
    public static async Task<double> Average(int[] array, object param)
    {
        Console.WriteLine("Multiples of 33:\n");
    ...
    public static async Task<int[]> Genrate33List(object param)
    {
